...
class PlaybackRequestMatcher {
  ...
  getResponse(responseId, interceptedRequestUrl) {
    let responseCollection = this.#responseCollections.get(responseId);

    if (!responseCollection) {
      throw new Error(`No response collection found with ID: ${responseId} and URL: ${interceptedRequestUrl}`);
    }
  ...
  isPending() {
    ...
    for (const response of this.#responseCollections.values()) {
      hits += response.hits;
    }
