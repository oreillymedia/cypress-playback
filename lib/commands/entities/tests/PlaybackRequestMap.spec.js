...
describe('PlaybackRequestMap', () => {
  beforeEach(() => {
    ...
    resetModule(/entities\/PlaybackResponseCollection.js$/);
  ...
  describe('deserialization', () => {
    it('works', () => {
      ...
      td.when(getInterceptedResponseId({ stuff: 1 }, td.matchers.anything()))
        .thenReturn(serializedMap.matchers[0].responseCollections[0].id);
      td.when(getInterceptedResponseId({ stuff: 2 }, td.matchers.anything()))
        .thenReturn(serializedMap.matchers[0].responseCollections[1].id);
